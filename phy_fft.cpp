...
WORD16 double2short(DOUBLE64 d)
{
	d = floor(0.5 + d);
...
void init_twiddle_factor(WORD16 * r2048twiddle, WORD16 * r32twiddle, WORD16* r64twiddle)
{
    WORD32 i, j;
	...
	for (i=0; i<32; i++)
	{
		for (j=0; j<64; j++)
		{
			*(WORD16 *)(r2048twiddle + i*2*64 + 2*j + 0) = double2short( sign * factor * sin(-2.0 * PI / 2048 * (i+0.5) * j));
			*(WORD16 *)(r2048twiddle + i*2*64 + 2*j + 1) = double2short( sign * factor * cos(-2.0 * PI / 2048 * (i+0.5) * j));
			sign *= -1;
		}
	}
	...
	for (i=0; i<8; i++)
	{
		for (j=0; j<4; j++)
		{
			*(WORD16 *)(r32twiddle + i*2*4*4 + 2*4*j + 0) = double2short( factor * sin(-2.0 * PI / 32 * i * (j+0.5)));
			*(WORD16 *)(r32twiddle + i*2*4*4 + 2*4*j + 1) = double2short( factor * cos(-2.0 * PI / 32 * i * (j+0.5)));

			*(WORD16 *)(r32twiddle + i*2*4*4 + 2*4*j + 2) = double2short( factor * sin(-2.0 * PI / 32 * i * (j+0.5)));
			*(WORD16 *)(r32twiddle + i*2*4*4 + 2*4*j + 3) = double2short( factor * cos(-2.0 * PI / 32 * i * (j+0.5)));

			*(WORD16 *)(r32twiddle + i*2*4*4 + 2*4*j + 4) = double2short( factor * sin(-2.0 * PI / 32 * i * (j+0.5)));
			*(WORD16 *)(r32twiddle + i*2*4*4 + 2*4*j + 5) = double2short( factor * cos(-2.0 * PI / 32 * i * (j+0.5)));

			*(WORD16 *)(r32twiddle + i*2*4*4 + 2*4*j + 6) = double2short( factor * sin(-2.0 * PI / 32 * i * (j+0.5)));
			*(WORD16 *)(r32twiddle + i*2*4*4 + 2*4*j + 7) = double2short( factor * cos(-2.0 * PI / 32 * i * (j+0.5)));
		}
	}
}

const static __m128i  FFT2048_IQ_switch = _mm_setr_epi8(2,3,0,1,6,7,4,5,10,11,8,9,14,15,12,13);
const static __m128i  FFT2048_Neg_I = _mm_setr_epi8(0x1, 0x1, 0xFF, 0xFF, 0x1, 0x1, 0xFF, 0xFF, 0x1, 0x1, 0xFF, 0xFF, 0x1, 0x1, 0xFF, 0xFF );
const static __m128i  FFT2048_Neg_R = _mm_setr_epi8(0xFF, 0xFF, 0x1, 0x1, 0xFF, 0xFF, 0x1, 0x1, 0xFF, 0xFF, 0x1, 0x1, 0xFF, 0xFF, 0x1, 0x1);
const static __m128i  FFT2048_Const_0707 = _mm_setr_epi8(0x7F, 0x5A, 0x7F, 0x5A, 0x7F, 0x5A, 0x7F, 0x5A,0x7F, 0x5A, 0x7F, 0x5A, 0x7F, 0x5A, 0x7F, 0x5A);
const static __m128i  FFT2048_Const_0707_Minus = _mm_setr_epi8(0x81, 0xA5,0x81, 0xA5,0x81, 0xA5,0x81, 0xA5,0x81, 0xA5,0x81, 0xA5,0x81, 0xA5,0x81, 0xA5);
...
{ \
	...
	m128_t4 = _mm_shuffle_epi8(m128_t3, FFT2048_IQ_switch); \
	m128_t4 = _mm_sign_epi16(m128_t4, FFT2048_Neg_I); \
	 \
...
{ \
	...
	m128_t12 = _mm_shuffle_epi8(m128_t12, FFT2048_IQ_switch); \
	m128_t12 = _mm_sign_epi16(m128_t12, FFT2048_Neg_R); \
	 \
...
{ \
	...
	m128_t5 = _mm_subs_epi16(m128_t3, m128_t7); \
	 \
	m128_t1 = _mm_shuffle_epi8(m128_t1, FFT2048_IQ_switch); \
	m128_t1 = _mm_sign_epi16(m128_t1, FFT2048_Neg_I); \
	 \
	m128_t2 = _mm_mulhrs_epi16(m128_t11, FFT2048_Const_0707); \
	m128_t3 = _mm_mulhrs_epi16(m128_t5, FFT2048_Const_0707_Minus); \
	 \
	m128_t6 = _mm_shuffle_epi8(m128_t2, FFT2048_IQ_switch); \
	m128_t7 = _mm_adds_epi16(m128_t2, m128_t6); \
	m128_t2 = _mm_subs_epi16(m128_t2, m128_t6); \
	m128_t11 = _mm_blend_epi16(m128_t2, m128_t7, 0x55); \
	 \
	m128_t6 = _mm_shuffle_epi8(m128_t3, FFT2048_IQ_switch); 	m128_t7 = _mm_adds_epi16(m128_t3, m128_t6); \
...
{ \
	...
	m128_t12 = _mm_shuffle_epi8(m128_t12, FFT2048_IQ_switch); \
	m128_t12 = _mm_sign_epi16(m128_t12, FFT2048_Neg_R);/*j*D*/ \
	 \
	...
	m128_t4 = _mm_shuffle_epi8(m128_t0, FFT2048_IQ_switch); \
	m128_t4 = _mm_sign_epi16(m128_t4, FFT2048_Neg_I); \
...
{ \
	...
	m128_t1 = _mm_shuffle_epi8(m128_t1, FFT2048_IQ_switch); \
	m128_t1 = _mm_sign_epi16(m128_t1, FFT2048_Neg_I); /* Temp8_5 */ \
	 \
	m128_t2 = _mm_mulhrs_epi16(m128_t11, FFT2048_Const_0707); /*Temp8_3*/ \
	m128_t3 = _mm_mulhrs_epi16(m128_t5, FFT2048_Const_0707_Minus);  /*Temp8_7*/ \
	 \
	m128_t6 = _mm_shuffle_epi8(m128_t2, FFT2048_IQ_switch); /*Temp8_3*/ \
	m128_t7 = _mm_adds_epi16(m128_t2,m128_t6); \
	m128_t2 = _mm_subs_epi16(m128_t2,m128_t6); \
	m128_t11 = _mm_blend_epi16(m128_t2,m128_t7, 0x55); \
	 \
	m128_t6 = _mm_shuffle_epi8(m128_t3, FFT2048_IQ_switch); /*Temp8_7*/ \
...
{ \
	radix8_0(InBuf + 0*in_span, 8*in_span, Temp64_32_Buf +0*1,8); \
	DIV_4_2_span((__m128i *)(Temp64_32_Buf) + 0*8,8); \
	DIV_4_2_span((__m128i *)(Temp64_32_Buf) + 4*8,8); \
	radix8_0_mul(InBuf + 1*in_span, 8*in_span, Temp64_32_Buf +1, 8, r64twiddle+1*8); \
	radix8_0_mul(InBuf + 2*in_span, 8*in_span, Temp64_32_Buf +2, 8, r64twiddle+2*8); \
	radix8_0_mul(InBuf + 3*in_span, 8*in_span, Temp64_32_Buf +3, 8, r64twiddle+3*8); \
	radix8_0_mul(InBuf + 4*in_span, 8*in_span, Temp64_32_Buf +4, 8, r64twiddle+4*8); \
	radix8_0_mul(InBuf + 5*in_span, 8*in_span, Temp64_32_Buf +5, 8, r64twiddle+5*8); \
	radix8_0_mul(InBuf + 6*in_span, 8*in_span, Temp64_32_Buf +6, 8, r64twiddle+6*8); \
	radix8_0_mul(InBuf + 7*in_span, 8*in_span, Temp64_32_Buf +7, 8, r64twiddle+7*8); \
	 \
	radix8_0(Temp64_32_Buf + 0*8, 1, OutBuf + 0*out_span, 8*out_span); \
	radix8_0(Temp64_32_Buf + 1*8, 1, OutBuf + 1*out_span, 8*out_span); \
	radix8_0(Temp64_32_Buf + 2*8, 1, OutBuf + 2*out_span, 8*out_span); \
	radix8_0(Temp64_32_Buf + 3*8, 1, OutBuf + 3*out_span, 8*out_span); \
	radix8_0(Temp64_32_Buf + 4*8, 1, OutBuf + 4*out_span, 8*out_span); \
	radix8_0(Temp64_32_Buf + 5*8, 1, OutBuf + 5*out_span, 8*out_span); \
	radix8_0(Temp64_32_Buf + 6*8, 1, OutBuf + 6*out_span, 8*out_span); \
	radix8_0(Temp64_32_Buf + 7*8, 1, OutBuf + 7*out_span, 8*out_span); \
}

#define radix4_0_zeromul_cpy(in_addr, in_span, out_addr, in_addr_cpy) \
{ \
	__m128i * in_addr_temp; \
	...
	_mm_store_si128((__m128i *)in_addr_cpy_temp, m128_t3);  \
	 \
	m128_t2 = _mm_shuffle_epi8(m128_t2, FFT2048_IQ_switch); \
	m128_t2 = _mm_sign_epi16(m128_t2, FFT2048_Neg_I); \
	 \
	m128_t12 = _mm_mulhrs_epi16(m128_t1, FFT2048_Const_0707); \
	m128_t10 = _mm_shuffle_epi8(m128_t12, FFT2048_IQ_switch); \
	...
	m128_t12 = _mm_blend_epi16(m128_t12, m128_t11, 0x55); \
	 \
	m128_t13 = _mm_mulhrs_epi16(m128_t1, FFT2048_Const_0707_Minus); \
	m128_t10 = _mm_shuffle_epi8(m128_t13, FFT2048_IQ_switch); \
	...
	m128_t13 = _mm_blend_epi16(m128_t13, m128_t11, 0xAA); \
	 \
	m128_t14 = _mm_mulhrs_epi16(m128_t3, FFT2048_Const_0707); \
	m128_t10 = _mm_shuffle_epi8(m128_t14, FFT2048_IQ_switch); \
	...
	m128_t14 = _mm_blend_epi16(m128_t14, m128_t11, 0x55); \
	 \
	m128_t15 = _mm_mulhrs_epi16(m128_t3, FFT2048_Const_0707_Minus); \
	m128_t10 = _mm_shuffle_epi8(m128_t15, FFT2048_IQ_switch); \
...
#define radix4_0_mul_cpy(in_addr, in_span, out_addr, twiddle_addr, in_addr_cpy) \
{ \
	__m128i * in_addr_temp; \
	__m128i * in_addr_cpy_temp; \
	__m128i * out_addr_temp; \
	...
	m128_t3 = _mm_shuffle_epi8(m128_t2, FFT2048_IQ_switch); \
	m128_t3 = _mm_sign_epi16(m128_t3, FFT2048_Neg_I); \
	m128_t5 = _mm_shuffle_epi8(m128_t4, FFT2048_IQ_switch); \
	m128_t5 = _mm_sign_epi16(m128_t5, FFT2048_Neg_I); \
	m128_t7 = _mm_shuffle_epi8(m128_t6, FFT2048_IQ_switch); \
	m128_t7 = _mm_sign_epi16(m128_t7, FFT2048_Neg_I); \
	m128_t15 = _mm_shuffle_epi8(m128_t14, FFT2048_IQ_switch); \
	m128_t15 = _mm_sign_epi16(m128_t15, FFT2048_Neg_I); \
	 \
	out_addr_temp = out_addr; \
...
#define radix32_cpy(InBuf, in_span, Temp64_32_Buf, r32twiddle, InBufCpy, OutBuf) \
{ \
	radix4_0_zeromul_cpy(InBuf + 0*in_span, 8*in_span, Temp64_32_Buf +0*4, InBufCpy + 0*in_span); \
	radix4_0_mul_cpy(InBuf + 1*in_span, 8*in_span, Temp64_32_Buf + 1*4, r32twiddle + 4*1, InBufCpy + 1*in_span); \
	radix4_0_mul_cpy(InBuf + 2*in_span, 8*in_span, Temp64_32_Buf + 2*4, r32twiddle + 4*2, InBufCpy + 2*in_span); \
	radix4_0_mul_cpy(InBuf + 3*in_span, 8*in_span, Temp64_32_Buf + 3*4, r32twiddle + 4*3, InBufCpy + 3*in_span); \
	radix4_0_mul_cpy(InBuf + 4*in_span, 8*in_span, Temp64_32_Buf + 4*4, r32twiddle + 4*4, InBufCpy + 4*in_span); \
	radix4_0_mul_cpy(InBuf + 5*in_span, 8*in_span, Temp64_32_Buf + 5*4, r32twiddle + 4*5, InBufCpy + 5*in_span); \
	radix4_0_mul_cpy(InBuf + 6*in_span, 8*in_span, Temp64_32_Buf + 6*4, r32twiddle + 4*6, InBufCpy + 6*in_span); \
	radix4_0_mul_cpy(InBuf + 7*in_span, 8*in_span, Temp64_32_Buf + 7*4, r32twiddle + 4*7, InBufCpy + 7*in_span); \
	 \
...
{ \
	...
	{ \
		...
		m128_t6 = _mm_load_si128((__m128i *)twiddle_addr_temp); \
		m128_t1 = _mm_shuffle_epi8(m128_t0, FFT2048_IQ_switch); \
		m128_t1 = _mm_sign_epi16(m128_t1, FFT2048_Neg_I); \
		m128_t3 = _mm_shuffle_epi8(m128_t2, FFT2048_IQ_switch); \
		m128_t3 = _mm_sign_epi16(m128_t3, FFT2048_Neg_I); \
		m128_t5 = _mm_shuffle_epi8(m128_t4, FFT2048_IQ_switch); \
...
/***************************************************************************//**
 * @fn void fft2048_core(__m128i *InBuf, __m128i *OutBuf, __m128i *Temp64_32_Buf,  __m128i *r32twiddle, __m128i *r64twiddle, __m128i *r2048twiddle)
 * @brief Perform the FFT2048
 * @param [in] InBuf Pointer of input buffer
 * @param [out] OutBuf Pointer of output buffer
 * @param [out] Temp64_32_Buf Pointer of tempory buffer
 * @param [in] r32twiddle Pointer of twiddle factor for radix32
 * @param [in] r64twiddle Pointer of twiddle factor for radix64
 * @param [in] r2048twiddle Pointer of twiddle factor for radix2048
 *******************************************************************************/
void fft2048_core(__m128i *InBuf, __m128i *OutBuf,  __m128i *r32twiddle, __m128i *r64twiddle, __m128i *r2048twiddle)
{
    WORD32 i_loop=0;
	...
	{
		radix32((__m128i *)(InBuf) + i_loop, in_span, (__m128i *)Temp64_32_Buf, (__m128i *)r32twiddle, (__m128i *)(OutTmp) + i_loop);
	}
	...
	transpose64x4_mulzero((__m128i *)(OutTmp) + 0*64, in_span, Temp64_32_Buf, r2048twiddle + 0*64);
	radix64(Temp64_32_Buf, in_span,(__m128i *)OutBuf + 0, out_span, (__m128i *)r64twiddle);

	for (i_loop=1;i_loop<8;i_loop++)
	{
		transpose64x4((__m128i *)(OutTmp) + i_loop*64, in_span, Temp64_32_Buf, r2048twiddle + i_loop*64);
		radix64(Temp64_32_Buf, in_span,(__m128i *)OutBuf + i_loop, out_span, (__m128i *)r64twiddle);
	}
}
...
void fft2048_core_cpy(__m128i *InBuf, __m128i *OutBuf, __m128i *r32twiddle, __m128i *r64twiddle, __m128i *r2048twiddle, __m128i *InBufCpy)
{
	...
	for (i_loop=0;i_loop<16;i_loop++)
	{
		radix32_cpy((__m128i *)(InBuf) + i_loop, in_span, (__m128i *)Temp64_32_Buf, (__m128i *)r32twiddle, (__m128i *)(InBufCpy) + i_loop, (__m128i *)(OutTmp) + i_loop);
	}
